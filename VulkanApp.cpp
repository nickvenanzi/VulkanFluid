...
{
    std::array<float, 8> phis = {1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f};
    ...
    for (uint32_t k = 0; k < 2; k++)
    {
        for (uint32_t j = 0; j < 2; j++)
        {
            for (uint32_t i = 0; i < 2; i++)
            {
                ...
                glm::vec3 color = phis[index] > 0.0f ? AIR_COLOR : WATER_COLOR;
                addCube(cubeSize, {offset[i], offset[j], offset[k]}, color);
            }
        }
    }

    constructSurface(phis);
}
...
void VulkanApp::constructSurface(const std::array<float, 8> &phis)
{
    glm::vec3 cubePosition = {-0.5f, -0.5f, -0.5f}; // replace with gridCell position
